...
\pdfinfo{
/Title (A-MSB-GRPO: Adaptive Multi-Layer Semantic-Balanced GRPO for Enhanced LLM Reasoning)
/Author (Minh Nguyen Quoc, Bang Nguyen Dinh, Bao Tran Dang)
}

\title{A-MSB-GRPO: Adaptive Multi-Layer Semantic-Balanced GRPO \\ for Enhanced LLM Reasoning}
...
\subsection{Formal Derivation of A-MSB-GRPO}
Let $q$ be a prompt and $O = \{o_1, \dots, o_N\}$ be $N$ rollouts from policy $\pi_\theta$. The standard reward for $o_i$ is $r_i \in \{0, 1\}$. In A-MSB-GRPO, we define the accuracy ratio $R = \frac{1}{N} \sum_{i=1}^N r_i$.
...
\begin{algorithm}[H]
\caption{A-MSB-GRPO Training Step}
\begin{algorithmic}[1]
...
All models were evaluated using the \texttt{lighteval} framework on the exact same hardware configuration to ensure fairness. Table \ref{tab:main} compares A-MSB-GRPO with vanilla GRPO (which lacks reflection entirely) and M-GRPO (which utilizes reflection but samples revisions uniformly without semantic clustering).
...
\begin{tabular}{lccc}
\toprule
Dataset & Vanilla GRPO & M-GRPO & \textbf{A-MSB-GRPO} \\
\midrule
...
\begin{table}[h]
\centering
\caption{MATH Sub-category Analysis: M-GRPO vs A-MSB-GRPO.}
\label{tab:math}
\begin{tabular}{l|cc}
\toprule
...
The results show that A-MSB-GRPO's strength lies in high-complexity reasoning. In the MATH dataset, our method provides a \textbf{2.46\%} absolute gain over M-GRPO. As shown in Table \ref{tab:math}, A-MSB-GRPO yields the most significant improvements in sub-categories requiring deep, multi-step logical derivations, such as \textit{Intermediate Algebra (+4.88\%)} and \textit{Precalculus (+5.68\%)}. For simpler or saturated categories like \textit{Algebra} (~89\%), the semantic profiling dampens unnecessary reflections, maintaining performance parity (-0.25\%). This is largely due to the \textbf{Targeted Error Correction} provided by SEP, proving that K-Means clustering effectively addresses underlying systematic errors rather than just applying pattern matching.

\textbf{Case Study:} For a complex geometry problem, M-GRPO initially generated a correct rollout but "over-corrected" in Layer 2, changing the answer. In contrast, A-MSB-GRPO's \textbf{Heuristic Confidence Gate} and \textbf{Entropy Scaling} identified the high certainty of the Layer 1 response, dampening the Layer 2 update and preserving the correct solution.

\subsection{Ablation Analysis: Deconstructing the Performance Gains}
Because our method builds upon fundamental RL and reflection techniques, our primary baselines inherently serve as functional ablations of the complete A-MSB-GRPO pipeline. By analyzing the performance deltas in Table \ref{tab:main}, we can isolate the contributions of specific modules:
