...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=\textwidth]{SB-GRPO-pipeline-v10.pdf}
...
To effectively disambiguate structural reasoning flaws from superficial generation noise, the SEP module projects all incorrect trajectories into a dense semantic space. Specifically, incorrect samples are vectorized utilizing a frozen \textit{all-MiniLM-L6-v2} encoder \cite{reimers2019sentencebertsentenceembeddingsusing}. Subsequently, K-Means clustering \cite{article} (empirically set to $K=4$) is applied to partition these latent embeddings into a discrete set of semantic clusters $C = \{C_1, \dots, C_K\}$. By identifying the top error cluster, defined mathematically as $C_{top} = \arg\max_k |C_k|$, we isolate the most prevalent logical failure mode associated with the given prompt. This isolation provides a concentrated learning signal for targeted error correction, ensuring the policy's optimization against systematic misconceptions rather than stochastic, trivial errors.
